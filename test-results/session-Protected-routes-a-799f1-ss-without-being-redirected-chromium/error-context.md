...
- Name: session.spec.ts >> Protected routes >> authenticated user can access / without being redirected
- Location: e2e\tests\session.spec.ts:67:7
...
Locator: getByRole('heading', { name: 'Welcome to PEELP' })
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByRole('heading', { name: 'Welcome to PEELP' })
